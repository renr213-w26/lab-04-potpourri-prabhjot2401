...
``` 
setting error to true allowed the document to render even though gplot function is unavailable, setting it to false doesn't allow the document to render
```
...
```Data is not provided, hence question cannot be completed
```
...
```{r}
datasaurus_dozen|>group_by(dataset)|>summarise(meanx=mean(x),meany=mean(y),stdx=sd(x),stdy=sd(y),corr=cor(x,y))|>print()
```
```
...
ggplot(datasaurus_dozen, aes(x,y, colour = dataset))+
  ...
  facet_wrap(~dataset)+labs(title="Scatterplot of Y vs X",subtitle="faceted by Dataset")
